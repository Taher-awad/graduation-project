...
from models import Room, RoomMember, User, UserRole, RoomMemberStatus
from schemas import RoomCreate, RoomResponse, InviteCreate, InvitationResponse
...
def create_room(room: RoomCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Add owner as member (optional, but good for listing logic)
    # Actually owner logic is separate usually, but let's keep it simple.
